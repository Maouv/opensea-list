...
async function fulfillData(orderHash, chain, protocol, seller, ca, tokenId, bearer, apiKey) {
  ...
  if (!r.ok) throw new Error(j.detail || j.message || `fulfillment_data ${r.status}`);
...
module.exports = { listOffers, listOffersWithOrders, offerLooksFundable, fulfillData, encodeFulfillment, acceptOffer, fmtOffer };
