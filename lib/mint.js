...
async function computeStageEligibility(drop, provider, ca, wallets, sig, slug, apiKey) {
  ...
  if (slug && apiKey && wallets.length && typeof wallets[0] === 'object') {
    try {
      osMatrix = await oselig.walletsEligibility(apiKey, slug, wallets);
    } catch (e) {
      osMatrix = null;
    }
  }
  return Promise.all(drop.stages.map(async (s) => {
    ...
    if (osMatrix) {
      if (s.type === 'PUBLIC_SALE') {
        return { stage: s, state, total, count: total, note: 'public, schedulable' };
      }
      const okCount = osMatrix.filter((w) => w.stages[s.index] === true).length;
      return {
        stage: s, state, total,
        count: okCount,
        reasons: osMatrix.map((w) => ({ minter: w.address, ok: w.stages[s.index] === true, reason: w.ok ? '' : (w.error || 'OS API failed') })),
        note: okCount > 0 ? `${okCount}/${total} eligible (OS)` : '0 eligible (OS)',
      };
    }
    if (state === 'upcoming') {
...
function describeStages(drop, elig) {
  ...
  const lines = drop.stages
    ...
    .map((s, i) => {
      ...
      const head = `${state === 'ACTIVE' ? '>' : ' '} ${i + 1}. [${typeShort[s.type] || s.type}] ${s.label || '?'}: ${price}, max ${s.maxPerWallet ?? '?'}/wallet`;
      let line = `${head}\n   ${fmtRangeWIB(s.start, s.end)} WIB (${state})${wl}`;
      const e = elig ? elig.find((x) => x.stage.index === s.index) : null;
      if (e) line += `\n   elig: ${e.count == null ? '?' : `${e.count}/${e.total}`} — ${e.note}`;
      return line;
    });
