...
function describeStages(drop) {
  ...
  const lines = drop.stages
    ...
    .map((s) => {
      ...
      const time = `${new Date(s.start).toISOString().slice(5, 16).replace('T', ' ')}-${new Date(s.end).toISOString().slice(5, 16).replace('T', ' ')}Z`;
      return `${state === 'ACTIVE' ? '>' : ' '} [${typeShort[s.type] || s.type}] ${s.label || '?'}: ${price}, max ${s.maxPerWallet ?? '?'}/wallet, ${time} (${state})${wl}`;
    });
  return lines.join('\n');
}
...
module.exports = { detect, checkWithPrice, collectionName, mintArgs, fetchDrop, describeStages, activeStage };
