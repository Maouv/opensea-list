  ...
  const lines = drop.stages
    ...
    .map((s, i) => {
      ...
      if (e) {
        ...
        if (e.count == null) line += `\n   elig: ? — ${e.note}`;
        else if (e.count === 0) line += `\n   elig: 0/${e.total} — 0 eligible${e.note === 'OS' ? ' (OS)' : ` — ${e.note}`}`;
        else if (e.count === e.total) line += `\n   elig: ${e.count}/${e.total}${e.note === 'public, schedulable' ? ' — public, schedulable' : ''}`;
        else if (e.count <= 3) line += `\n   elig: ${e.count}/${e.total} — ${e.reasons.filter((r) => r.ok).map((r) => short(r.minter)).join(', ')}`;
        else line += `\n   elig: ${e.count}/${e.total} — tap [Elig wallet]`;
      }
      return line;
    });
