...
If the collection is an OpenSea drop, the bot scrapes the stage schedule from the collection page and shows it in WIB (sep 24 19.30 - 20.00 format, supply header, allowlist sizes). When a stage is active, the wallet picker simulates the mint for every wallet on-chain and marks each one eligible or not with the revert reason. When no stage is open, the schedule view offers Set schedule mint: pick an upcoming PUBLIC stage (WL/signed stages need an OpenSea signature and are not supported), then Bulk-mint (enter how many wallets, filled in order), Separate-mint (paginated 5-per-page wallet list, select by typing numbers, toggle off by re-typing), or See all wallet. Per-wallet quantity is only asked when the stage allows more than 1. After confirmation the schedule is stored in mint-schedules.json and the bot fires automatically at the stage start time, re-reading the live price from the drop page before minting. Schedules that were pending while the bot was down and whose stage already started are marked missed; a pending schedule with a future start re-arms on boot.
